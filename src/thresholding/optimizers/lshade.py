...
Uses a deterministic Linear Population Size Reduction scheme
Dynamically decreases the population size N linearly over MAX_FES

Remaining parameter history adapation follows SHADE
"""
...
class LSHADE(BaseOptimizer):
    ...
    def __init__(self, *args, H: int = 6, p: float = 0.11, r_arc: float = 2.6, N_min: int = 4, **kwargs):
        super().__init__(*args, **kwargs)
        if kwargs.get("pop_size") is None:
            self.pop_size = 18 * self.dim
        self.H = H
        self.p = p
        self.r_arc = r_arc
        self.N_min = N_min
    ...
    def optimize(self) -> OptResult:  # noqa: D401
        ...
        while evals < self.max_fes:
            ...
            #Compute target size
            N_next = int(round((self.N_min - N_init) / self.max_fes * evals + N_init))
            N_next = max(self.N_min, N_next)

            if N_next < N:
                keep = np.argsort(-fit)[:N_next]  #best individuals
                pop, fit = pop[keep], fit[keep]
                N = N_next

            #Trim archive
            arc_cap = int(round(self.r_arc * N))
